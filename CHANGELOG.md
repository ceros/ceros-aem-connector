...
- Server-side renders now emit an import map resolving `@ceros/flex-experience-sdk`, so `type="module"` scripts in the injected custom Body HTML can import the SDK by name. Ceros renders an import map only on the standalone published page, so one is supplied here; it is added only when the injected HTML actually imports the SDK, leaving the page's single allowed import map free otherwise. The SDK module loads without SRI, as the experience manifest does not publish its hash.
