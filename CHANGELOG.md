...
### Security
- Pasted experience URLs are no longer trusted by default. A manifest is only fetched — and the scripts it references only injected — when it is served from a **Ceros-owned** domain (`ceros.com`, `ceros.site` by default; configurable). Authors may still paste any URL: a customer **vanity domain** is resolved by reading the new [`x-flex-manifest`](https://github.com/ceros/ceros-spark/pull/9861) header off the published page to discover its canonical, Ceros-hosted manifest URL, and the advertised URL must itself pass the Ceros-owned whitelist (so a spoofed header pointing off-Ceros is rejected). Mirrors the WordPress connector's keyless paste hardening ([ceros-plugin-wordpress#3](https://github.com/ceros/ceros-plugin-wordpress/pull/3)).
  - New `CerosManifestService.resolveTrustedManifestUrl` performs the whitelist + `x-flex-manifest` resolution. Pasted URLs are validated at authoring time: the store **Fetch** servlet resolves synchronously, and the dialog post-processor canonicalises the stored `manifestUrl` for the live **inline** and **fetch** modes on save — so render trusts the stored URL and makes no extra network call. `fetchPublicManifestFromUrl` enforces the Ceros-owned whitelist as a defence-in-depth choke point.
  - New OSGi config on `CerosManifestServiceImpl`: `cerosOwnedDomains` (trusted apex domains; **production domains only by default** — `cerosdev.site` / `cerosstage.site` are excluded so customer installs never reference internal environments, and are added back via the author-SDK config for local/dev) and `allowUntrustedManifestHost` (dev/test relaxation, enabled in the author-SDK config so localhost manifests still work; **off** in production).
