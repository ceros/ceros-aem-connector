 ...
 * Store mode's Fetch button already does. Mirrors the URL validation of
 * {@link CerosFlexManifestUrlPostProcessor} (the server-side gate that ultimately
 ...
 * recognized Ceros experience; {@code 502 {message}} when the experience can't
 * be reached.
 */
@Component(
...
public class CerosManifestValidateServlet extends SlingAllMethodsServlet {
    ...
    protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response)
        ...
        try {
            String manifestUrl = cerosManifestService.resolveTrustedManifestUrl(incomingUrl);
            ServletUtils.writeJson(response, Map.of("valid", true, "manifestUrl", manifestUrl));
        } catch (IllegalArgumentException e) {
            log.debug("Rejected manifest URL {}: {}", incomingUrl, e.getMessage());
            ServletUtils.writeError(response, SlingHttpServletResponse.SC_BAD_REQUEST, e.getMessage());
        } catch (IOException e) {
            log.debug("Could not reach experience to verify {}: {}", incomingUrl, e.getMessage());
